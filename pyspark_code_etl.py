# Code for Pyspark ETL job which aims to transform data and write the output in parquet files for the regions "ca", "gb" and "us"
...
datasource0 = glueContext.create_dynamic_frame.from_catalog(database = "de-youtube-raw-data", table_name = "raw_statistics_reference_data", transformation_ctx = "datasource0", push_down_predicate = predicate_pushdown)
...
datasink4 = glueContext.write_dynamic_frame.from_options(frame = df_final_output, connection_type = "s3", connection_options = {"path": "s3://de-youtube-cleansed-data-nb/youtube/raw-statistics-reference-data/", "partitionKeys": ["region"]}, format = "parquet", transformation_ctx = "datasink4")
